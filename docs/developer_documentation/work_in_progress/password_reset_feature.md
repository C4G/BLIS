...
ToolsWindowViewModel is a lightweight shell that holds PasswordResetViewModel as a property.
ServerControlViewModel receives ToolsWindowViewModel via DI. When the user clicks the reset button, HandleOpenPasswordReset calls ResetForm() on the injected PasswordResetViewModel to clear any stale state from a previous session, then opens ToolsWindow as a modal dialog. The RequestClose callback is wired after the window is created so it can call toolsWindow.Close() directly.
Dependency injection : All four classes are registered as singletons in ServiceCollectionExtensions: PasswordResetViewModel, ToolsWindowViewModel, ServerControlViewModel
