...
3. Click "More Options" and select "Update with ZIP File" from the dropdown.
4. Select the downloaded ZIP file when prompted.
5. The launcher will proceed through the two-stage update process automatically. This includes staging the new files into a versioned release directory, stopping Apache and MySQL, backing up the database, updating `state.json` to reflect the new active version, and restarting services.
6. The BLIS PHP application will prompt the administrator to apply any pending database migrations through the existing migration process.
7. Verify the update succeeded by confirming the new version number is displayed on the launcher.

---

## Update Process Details

**Stage 1: Launcher Update**
The launcher extracts the contents of the ZIP file into a local staging directory. The launcher executable is then updated in-place. [`TODO` fill in brief description of the in-place .exe update mechanism.]

**Stage 2: BLIS Application Update**
The launcher halts Apache and MySQL, then stages the new application files into a versioned release directory. MySQL is restarted to generate a database backup. The `state.json` file is then updated to set the new version as active and the previous version as the prior release. The launcher rebuilds the httpd.conf file using a Liquid template that references the active version from `state.json`, and restarts Apache and MySQL to bring the application back online.
