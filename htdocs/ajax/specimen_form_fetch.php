	...
	?>
        <div class='result-holder'>
			<?php
				...
				{
			?>
					<select name='result[]' id='<?php echo $input_id; ?>' class='uniform_width' 
						onchange="javascript:update_remarks(<?php echo $test_type->testTypeId; ?>, <?php echo count($measure_list); ?> ,<?php echo $patient->getAgeNumber(); ?>, '<?php echo $patient->sex;?>');">
						<option></option>
						<?php
							foreach($range_values as $option)
							{
								...
								echo "<option value='$option'>".str_replace('#', '/', $option)."</option>";
							}
						?>
					</select>
			<?php
				...
				<?php 
					...
					{	
						...
						if($lower_parts[0]!=0)
						{
							echo $lower_parts[0].$units[0];
						}
						if($lower_parts[1]!=0)
						{
							echo $lower_parts[1].$units[1];
						}
						echo "-";
						if($upper_parts[0]!=0)
						{
							echo $upper_parts[0].$units[0];
						}
						if($upper_parts[1]!=0)
						{
							echo $upper_parts[1].$units[1];
						}
						echo ")";
					}else{	
						if(stripos($unit,":")!=false)
				...
				if($num_tests > 0 && $count == 0)
				{
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<small>
						<input type='checkbox' id='<?php echo $curr_form_id; ?>_skip' class="enable_check" 
							title='Tick this box if results are available and ready to enter' 
							onclick="javascript:toggle_form('<?php echo $curr_form_id; ?>', this);">
			...
			<tr>
				...
				<td>
					...
					<textarea name='comments' id='<?php echo $curr_form_id; ?>_comments'  class='uniform_width'
						onfocus="javascript:update_remarks(<?php echo $test_type->testTypeId; ?>, <?php echo count($measure_list); ?>, <?php echo $patient->getAgeNumber(); ?>, '<?php echo $patient->sex;?>');" ></textarea>
					</span></td>
			</tr>
			<tr>
				<td>
