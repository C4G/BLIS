...
else
{
	...
	{	
		if (isset($_REQUEST[($measure_names[$i])])||($measure_names[$i] == "")) 
		//if($measure_names[$i] == "")
		{
			# Mark for deletion
		...
		if($measure_types[$i] == Measure::$RANGE_NUMERIC)
		{
			...
			if(count($ranges_lower)==1)
			$reference_ranges_list[$i][] = array($ranges_lower[0], $ranges_upper[0], 0, 0,'B');
			
			else if(count($ranges_lower)==2)
			{
			$reference_ranges_list[$i][] = array($ranges_lower[0], $ranges_upper[0], 0, 0,'M');
			$reference_ranges_list[$i][] = array($ranges_lower[1], $ranges_upper[1], 0, 0,'F');
			}
			else
