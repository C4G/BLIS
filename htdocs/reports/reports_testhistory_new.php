...
{	copy($logo_path1,$logo_path);
	...
	<img src='<?php echo "logos/logo_".$lab_config_id.".jpg"; ?>' alt="Big Boat" height='140px' />
...
?>
	...
	<img src='<?php echo "logos/logo_".$lab_config_id.".jpg"; ?>' alt="Big Boat" height='140px' width='140px'/>
...
<table class='print_entry_border <?php if( $report_config->showBorder) echo "tblborder"; else echo "tblnoborder" ?>'>
			...
			if(is_array($ordered_fields))
			{
				...
				?>
                    <td class="heading "><?php  echo LangUtil::$generalTerms[$combined_fields[$field]]?></td>                        
                      <td><?php echo getFieldValue($pid[2],$previous_daily_num); ?></td><td width="15px" style="border:none"></td>
	...
	echo "<th width='500px' align='center'>Results</th>";
	...
	//showing of measure,results and range has been combined here( Hardcoded). For Nyaho Medical Center report format
	
	echo "<td class='rstyle' colspan='3'> <table cellspacing='0' cellpadding='0'>";// start new colspan
	$measurelist= $test->getMeasureListArray();
	$result_array=$test->getResultAsArray();
	for($i=0;$i<count($measurelist);$i++)
	{
		echo "<tr><td class='rstyle' width='250px'>".$measurelist[$i]."</td>";
		if(strstr($result_array[$i],"_") != FALSE)
		{
			/*$autocompleteRes = explode("_",$result_array[$i]);		
			for($j=0;$j<count($autocompleteRes);$j++)
			{
				echo "<td class='rstyle' width='150px'>".$autocompleteRes[$j]."</td>";
				
			}*/
			
			echo "<td class='rstyle' width='150px'>".str_replace("_", "<br/>", $result_array[$i])."</td>";
		}
		else
		{
			echo "<td class='rstyle' width='150px'>".$result_array[$i]."</td>";
		}
		
		echo "<td class='rstyle' width='100px'>";
		if($test->isPending() === true)
			echo "N/A";
		else
			{
				$test->getTestMeasureRange($measurelist[$i],$result_array[$i]);
			}
			echo "</td>";	
		
		echo "</tr>";
	}	
	echo "</table></td>";//close colspan 
	
	
	/*
	if($report_config->useMeasures == 1) {
		echo "<td class='rstyle' colspan='3'>";
	...
	if($report_config->useResults == 1) {
		...
		{			
			//echo $test->decodeResultWithoutMeasures(false,$patient);
			//$result_array=$test->getResultAsArray();	
		...
		echo "</td>";
...

				{
...

					{
						...
						//$test->getTestMeasureRange($result_array[0],
