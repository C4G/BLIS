...
$server_pubkey_text = base64_decode($server_pubkey->PubKey);
...
// TODO: Why does encryption not work??
...
// copy($tmppath, "C:\Users\beatl\Desktop\myzip.enc");
// $log->warn(base64_encode($iv));
// $log->warn(base64_encode($ekeys[0]));

$curl = curl_init();

if (function_exists('curl_file_create')) {
    // For PHP 5.5+, which is what we use in the BLIS docker image
    $curlfile = curl_file_create($latest_backup->full_path, 'application/octet-stream');
} else {
    // For old-school PHP, which is in-use by the Desktop BLIS
    $curlfile = '@' . realpath($latest_backup->full_path);
