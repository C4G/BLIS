...
public class SYSMEXXS500i extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("SYSMEX XS-500i handler stopped", DisplayMessageType.TITLE);
      ...
      @Override
    public void run() {
        log.AddToDisplay.Display("SYSMEX XS-500i handler started...", DisplayMessageType.TITLE);
        ...
        try
        {
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
                ...
                log.AddToDisplay.Display("SYSMEX XS-500i is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                ClientThread client = new ClientThread(connSock,"SYSMEX XS-500i");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("SYSMEX XS-500i client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
    ...
    {
        String data = "";
        xmlparser p = new xmlparser("configs/SYSMEX/SYSMEXXS500i.xml");
        try {
