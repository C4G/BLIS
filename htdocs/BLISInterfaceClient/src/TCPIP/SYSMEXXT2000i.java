 ...
 */
public class SYSMEXXT2000i extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            if(null !=welcomeSocket)
                 welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("SYSMEX XT-2000i handler stopped", DisplayMessageType.TITLE);
    ...
    public void run() {             
        ...
        try
        {
            log.AddToDisplay.Display("SYSMEX XT-2000i handler started...", DisplayMessageType.TITLE); 
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
            ...
            else
            {
               ...
               connSock = new Socket(tcpsettings.EQUIPMENT_IP, tcpsettings.PORT);
                ...
                log.AddToDisplay.Display("SYSMEX XT-2000i is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                if(!tcpsettings.SERVER_MODE)
                {
                    connSock.setKeepAlive(true);
                }
                ClientThread client = new ClientThread(connSock,"SYSMEX XT-2000i");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("SYSMEX XT-2000i client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
         ...
         try
         {
            ...
            {            
                ...
                {     
                       ...
                       log.AddToDisplay.Display("Sending test with Code: "+SampleList.get(i).aux_id + " to SYSMEX XT 2000i",DisplayMessageType.INFORMATION);
