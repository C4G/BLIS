...
{
	$l_strTime ="";
	if(stristr($age_to,"day") === false && stristr($age_to,"month") ===false)
	{
		$l_strTime =" Years";
	}
	
	$u_strTime ="";
	if(stristr($age_from,"day") === false && stristr($age_to,"month") ===false)
	{
		$u_strTime =" Years";
	}
	
    $query_string = "";	
    $l_age_s = "-".$age_to."$l_strTime";
    $l_age_t = strtotime($l_age_s);
    $l_age_d = date('Y-m-d', $l_age_t);
    $u_age_s = "-".$age_from."$u_strTime";
    $u_age_t = strtotime($u_age_s);
    $u_age_d = date('Y-m-d', $u_age_t);
	
	//echo $age_to.'|'.$l_age_d .' to '.$u_age_d;exit;
			...
			"AND specimen_id IN ( ".
				...
				"WHERE date_collected BETWEEN '$date_from' AND '$date_to' ".
                                "AND patient_id IN ( ".
                                    "SELECT patient_id from patient ".
                                    "WHERE sex = '$gender' ".
                                    "AND partial_dob BETWEEN '$l_age_d' AND '$u_age_d'".
                                    ")".
				" )";
    }
    else
