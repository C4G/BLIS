...
function get_edit_locale_form($lang_code, $lab_config_id)
{
    ...
    foreach ($default_lang_pages as $page_id => $default_terms) {
        ...
        $target_terms = $target_lang_pages[$page_id];
        ...
        foreach ($default_terms as $term_name => $default_term) {
            $target_term = $target_terms[$term_name];
            if (trim($default_term) == "") {
                continue;
            }
            echo "<tr valign='top'>";
            echo "<td title='$term_name'> $default_term </td>";
            if (strpos($term_name, "TIPS_") !== false) {
                echo "<td> <textarea class='no_specialchar uniform_width_more' name='$term_name' style='height:80px;'>$target_term</textarea> </td>";
            } else {
                echo "<td> <input class='no_specialchar uniform_width_more' type='text' name='$term_name' value='$target_term'></input></td>";
            }
            echo "</tr>";
        }
        echo "<tr>";
...
?>
        $('.no_specialchar').blur(function() {
            check_specialchar_alert(this);
        });
    });
        ...
        <tr>
            <td>
                ...
                <select name='lang_id' id='lang_id'>
                    <option value='en' <?php if ($locale == "en") {
                        echo " selected ";
                    } ?>>English</option>
                    <option value='fr' <?php if ($locale == "fr") {
                        echo " selected ";
                    } ?>>Francais</option>
                </select>
                &nbsp;&nbsp;&nbsp;
